...
(define (vis-binding key value)
  (when (not (symbol? key)) (raise-argument-error 'vis-binding "symbol?" 1))
  (when (not (vis-value? value)) (raise-argument-error 'vis-binding "vis-value?" 2))
  (cons key value) )
(define (vis-binding->key b)
  (when (not (vis-binding? b)) (raise-argument-error 'vis-binding->key "vis-binding?" 1))
  (car b))
(define (vis-binding->value b)
  (when (not (vis-binding? b)) (raise-argument-error 'vis-binding->value "vis-binding?" 1))
  (cdr b) )
...
(define (env-key-value->env env key value) (env-binding->env env (cons key value)))
...
(define (apply-vis-closure closure args)
(when (not (vis-closure? closure))
  (raise-argument-error 'apply-vis-closure "vis-closure?" 1) )
  (vis-eval
...
(define (eval-begin env s-expr-list)
  (when (not (pair? s-expr-list))
    (raise-argument-error 'eval-begin "non-empty-list?" 2) )
...
(define (key-env-env0->binding key env env0)
  (cond [(not (symbol? key)) (raise-argument-error 'key-env-env0->binding "symbol?" 1) ]
        [(not (vis-env? env)) (raise-argument-error 'key-env-env0->binding "vis-env?" 2) ]
        [(or (null? env) (eq? env0 env)) #f]
...
(define (eval-begin-eval env0 env s-exp)
  (when (not (vis-env? env0)) (raise-argument-error 'eval-begin-eval "vis-env?" 1))
  (when (not (vis-env? env)) (raise-argument-error 'eval-begin-eval "vis-env?" 2))
  (when (not (vis-value? s-exp)) (raise-argument-error 'eval-begin-eval "vis-value?" 3))
